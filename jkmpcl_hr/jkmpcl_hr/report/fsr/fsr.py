...
def get_data(filters):
	...
	# Grand total for the complete filtered report.
	grand_totals = {
		"visit_points": 0,
		"distance": 0.0,
		"tada_flat": 0.0,
	...
	for emp in employees:
		...
		# Subtotal for this employee for the complete filtered date range.
		emp_totals = {
			"visit_points": 0,
			"distance": 0.0,
			"tada_flat": 0.0,
		...
		for day in date_list:
			...
			emp_rows.append(row)
			print("Row for employee {} on {}: {}".format(emp.name, day, row))
			print("Bike Rate Cache for {} on {}: {}".format(emp.name, day, bike_rate_cache))

			# Add only numeric report values to the employee subtotal.
			emp_totals["visit_points"] += cint(row.get("visit_points") or 0)
			emp_totals["distance"] += flt(row.get("distance") or 0)
			emp_totals["tada_flat"] += flt(row.get("tada_flat") or 0)
			emp_totals["plant_vehicle_used"] += flt(row.get("plant_vehicle_used") or 0)
			emp_totals["total_ta_km"] += flt(row.get("total_ta_km") or 0)

		# Add this employee's values to the complete report total.
		for field in grand_totals:
			grand_totals[field] += emp_totals[field]
		...
		out.append({
			...
			"start_time": "",
			"end_time": "",
			"visit_points": emp_totals["visit_points"],
			"duration": "",
			"distance": flt(emp_totals["distance"], 2),
			"visit_type": "",
			"ta_da_mode": "",
			"tada_flat": flt(emp_totals["tada_flat"], 2),
			"plant_vehicle_used": flt(emp_totals["plant_vehicle_used"], 2),
			"total_ta_km": flt(emp_totals["total_ta_km"], 2),
			"photo_status": "",
			"is_subtotal_row": 1,
		})

	# Grand total row at the very end of the report.
	out.append({
		"employee": "",
		"employee_name": "",
		"date": _("GRAND TOTAL"),
		"start_time": "",
		"end_time": "",
		"visit_points": grand_totals["visit_points"],
		"duration": "",
		"distance": flt(grand_totals["distance"], 2),
		"visit_type": "",
		"ta_da_mode": "",
		"tada_flat": flt(grand_totals["tada_flat"], 2),
		"plant_vehicle_used": flt(grand_totals["plant_vehicle_used"], 2),
		"total_ta_km": flt(grand_totals["total_ta_km"], 2),
		"photo_status": "",
		"is_grand_total_row": 1,
	})
...
def get_bike_rate(on_date, bike_rate_cache):
	...
	"""
	print("Bike Rate Cache:", bike_rate_cache)
	if "rates" not in bike_rate_cache:
		...
		print("Bike Rate Cache:", bike_rate_cache["rates"])
...
def build_row(emp, day, attendance_map, rate_cache, cutoff_cache, bike_rate_cache):
	...
	print("DEBUG {} {} | visit_points={} before_cutoff={} eligible={} ta_mode={!r}".format(
		emp.name, day, visit_points, len(before_cutoff_visits), eligible_for_tada, ta_da_mode
	))
	...
	if eligible_for_tada:
		...
		if ta_mode == "Flat TA DA":
			...
			print("DEBUG: ta_mode {!r} matched NONE of the expected modes".format(ta_mode))
...
def export_excel_with_header(filters=None):
	...
	for row_idx, row in enumerate(data, start=1):
		...
		# Rows generated by get_data() are explicitly marked as
		# employee subtotal or final grand-total rows.
		is_subtotal_row = isinstance(row, dict) and row.get("is_subtotal_row")
		is_grand_total_row = isinstance(row, dict) and row.get("is_grand_total_row")
		is_total_row = is_subtotal_row or is_grand_total_row
		...
		if is_subtotal_row:
			worksheet.set_row(excel_row, 22)
		elif is_grand_total_row:
			worksheet.set_row(excel_row, 24)

	# Last data row is the grand-total row generated by get_data().
