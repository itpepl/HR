...
from frappe.utils import getdate, get_link_to_form, nowdate, get_datetime, add_days, date_diff # ✅ NEW IMPORT
...
def _get_holiday_type_for_date(employee: str, date) -> str | None:
    """
    Returns 'Weekly Off', 'Holiday', 'Restricted Holiday', or None.
    Standalone helper so we don't need to call get_holiday_type from
    scheduler_method inside create_auto_checkin_and_attendance synchronously.
    """
    date = getdate(date)
...
class AttendanceRequest(HRMSAttendanceRequest):
    ...
    def validate(self):
        ...
        self.validate_holiday_attendance_request()  # ✅ NEW
    ...
    #     # ✅ 1. Employee message → set ONLY once (never overwrite)
    #     if employee_msg and not self.custom_note:
    #         self.db_set("custom_note", employee_msg, update_modified=False)

    #     # ✅ 2. Approver message → always update (so latest attempt count shows)
    #     if approver_msg:
    ...
    # ─────────────────────────────────────────────────────────────────
    # ✅ NEW METHOD — Holiday restriction
    # ─────────────────────────────────────────────────────────────────
    def validate_holiday_attendance_request(self):
        """
        Block Attendance Request if the date falls on a Weekly Off /
        Holiday / Restricted Holiday and the employee does NOT have an
        approved Off-Day Work Request for that date.
        """

        from_date = getdate(self.from_date)
        to_date = getdate(self.to_date) if getattr(self, "to_date", None) else from_date

        current = from_date
        while current <= to_date:
            holiday_type = self._get_holiday_type(current)
            ...
            if holiday_type:
                ...
                if not approved:
                    frappe.throw(
                        _(
                            "Cannot create Attendance Request for {0} on {1}.<br>"
                            "This day is a <b>{2}</b>. "
                            "An approved <b>Off-Day Work Request</b> is required "
                            "before submitting an Attendance Request."
                        ).format(
                            frappe.bold(self.employee),
                            frappe.bold(current.strftime("%d-%m-%Y")),
                            _(holiday_type),
                        ),
                        title=_("Off-Day Work Request Required"),
                    )

            current = add_days(current, 1)

    def _get_holiday_type(self, date) -> str | None:
        """
        Returns "Weekly Off", "Holiday", "Restricted Holiday", or None.
        Uses get_current_holiday_list — same logic as attendance_scheduler.
        """
        date = getdate(date)

        holiday_list = get_current_holiday_list(self.employee, date)
    ...
    def create_auto_checkin_and_attendance(self):
        ...
        try:
            ...
            if in_time or out_time:
                ...
                # ✅ Do NOT call recalculate on holiday dates.
                # create_or_update_attendance_from_request only ENQUEUES the
                # background job — it does not run immediately. If we call
                # recalculate now with only 1 checkin in DB, working_hours=0
                # → status becomes Absent, overwriting the correct holiday status.
                holiday_type = _get_holiday_type_for_date(self.employee, self.from_date)
                if not holiday_type:
                    recalculate_attendance_after_manual_log(
                        self.employee,
                        self.from_date
...
def update_attendance_direct_db(employee, date, in_time, out_time, working_hours):
    ...
    # ✅ Check holiday FIRST — status must remain Holiday/Weekly Off/Restricted Holiday
    holiday_type = get_holiday_type(employee, date)

    # ✅ If existing attendance already has a holiday status, only update
    # timing fields — never touch the status
    HOLIDAY_STATUSES = {"Weekly Off", "Holiday", "Restricted Holiday"}
